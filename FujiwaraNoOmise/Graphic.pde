public class Graphic {
  ...
  public void drawTextField (MyShop ms, int selection, int howmany, int colorselection) {
    ...
    if (selection < ms.itemOnWindow.size()) {
      ...
      text("Estimated Value: " + (int)(howmany * ms.itemOnWindow.get(selection).price * (100 - ms.level) / 100.0), 70, 360);
  ...
  public void drawMyShopItems (MyShop ms, int emphasis) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (ms.itemOnWindow.size() <= i + 3 && ms.maxNum <= i + 3) {
        ...
        textSize(20);
        try {
          text("" + ms.itemOnWindow.get(i + 3).howmanyleft, 85 + 65 * i, 180);
          text("$" + ms.itemOnWindow.get(i + 3).price, 85 + 65 * i, 210);
        } catch (Exception e) {
        }
      }
    }
